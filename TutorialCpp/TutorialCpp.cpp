...
int main()
{
    ...
    CKDTree tree(3);
    DoubleVectors points =
    {
        {0.813837,0.0466323,0.758149},
        {0.814203,0.239998,0.869506},
        {0.897885,0.256508,0.91455},
        {0.269356,0.208014,0.879844},
        {0.874844, 0.887692,0.981674},
        {0.844783, 0.609272,0.993193 },
        {0.0408948, 0.173528,0.212784 }
    };
    ...
    CKDResult res;
    int cnt = tree.NearestRange(points[3].data(), 2, res);
    auto first = res.First();
    std::vector<intptr_t> idx;
    while (first)
    {
        idx.push_back(CKDTree::Index(first));
        first = res.Next();
    }
